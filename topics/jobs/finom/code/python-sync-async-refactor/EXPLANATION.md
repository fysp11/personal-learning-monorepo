...
From anywhere:
```bash
(cd topics/jobs/finom/code/python-sync-async-refactor && uv sync && uv run pytest -q)
```

To see visible output:
```bash
(cd topics/jobs/finom/code/python-sync-async-refactor && uv run python bad_sync_api.py --demo)
```

```bash
(cd topics/jobs/finom/code/python-sync-async-refactor && uv run python good_async_api.py --demo)
```

To run the API servers:
```bash
(cd topics/jobs/finom/code/python-sync-async-refactor && uv run python bad_sync_api.py --port 8001)
```

```bash
(cd topics/jobs/finom/code/python-sync-async-refactor && uv run python good_async_api.py --port 8002)
```
